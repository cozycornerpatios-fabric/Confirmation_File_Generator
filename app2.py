...
def build_pdf(job_id: str, payload: dict, base_url: str):
    ...
    try:
        ...
        DIAGRAM_SCALE = min(
            ...
            (SECTION_H - 0.1 * inch) / (4.5 * inch)  # assume original height
        )
        ...
        def draw_cushion_section(cnv, cushion, section_index_on_page):
            ...
            # --- left column: details
            x_details = MARGIN_L
            y_details = section_top_y - 0.1 * inch
            ...
            y_diagram_origin = section_bottom_y + 0.1 * inch  # tighter padding

            cnv.saveState()
            # Move origin to right column area and scale down
            cnv.translate(x_diagram_origin, y_diagram_origin)
            # Scale to fully utilize available space (up or down)
